...
export type LocaleCode = 'de' | 'en' | 'es' | 'fr' | 'pt';

const SUPPORTED_LOCALES: LocaleCode[] = ['de', 'en', 'es', 'fr', 'pt'];

type TemplateDictionary = Record<LocaleCode, string>;
...
type LocalizedTemplate = {
  titleKey: string;
  bodyKey: string;
  title: TemplateDictionary;
  body: TemplateDictionary;
};
...
const NOTIFICATION_TEMPLATES: Record<NotificationTemplateKey, LocalizedTemplate> = {
  'lead.new': {
...
const JOB_CHANGE_LABELS: Record<string, TemplateDictionary> = {
  date: {
...
export function getNotificationMessage(
  ...
  if (!template) {
    logger.warn('Missing notification template', { templateKey });
    return {
      title: 'Notification',
  ...
  const titleTemplate = template.title[targetLocale] ?? template.title.en;
  const bodyTemplate = template.body[targetLocale] ?? template.body.en;
...
export function formatJobChanges(
  locale: LocaleCode,
  changeKeys: string[]
): string {
...
function resolveTemplateParams(
  rawParams: NotificationTemplateParams,
  locale: LocaleCode,
  templateKey: NotificationTemplateKey
...
function formatDateForLocale(input: Date, locale: LocaleCode): string {
  try {
    ...
    logger.warn('Date formatting failed, using ISO string', { error });
    return input.toISOString();
  }
}

function formatCurrency(amount: number, locale: LocaleCode): string {
  try {
    ...
    logger.warn('Currency formatting failed, using fallback string', { error });
    return `${amount.toFixed(2)} €`;
  }
}

function joinWithConjunction(values: string[], locale: LocaleCode): string {
  if (values.length <= 1) {
    return values[0] ?? '';
  }

  const conjunctionWord: Record<LocaleCode, string> = {
    de: 'und',
    en: 'and',
    es: 'y',
    fr: 'et',
  ...
  const word = conjunctionWord[locale] ?? conjunctionWord.en;
  if (values.length === 2) {
